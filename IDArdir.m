function [varargout] = IDArdir(rootdir,varargin)
% Lists the files in a directory and its sub directories. 
% 
% function [D] = IDArdir(ROOT,TEST)
%
...
%   % the screen.
%   IDArdir('c:\program files\windows *\*.exe');
%   IDArdir('c:\program files\windows *\**\*.dll');
%
%   % Using the test function to find files modified today
%   IDArdir('c:\win*\*','datenum>floor(now)');
%   % Using the test function to find files of a certain size
%   IDArdir('c:\program files\win*\*.exe','bytes>1024 & bytes<1048576');
%
...
elseif strcmp(wildpath,'**'), % a double wild directory means recurs down into sub directories
  ...
  D = IDArdir([prepath postpath(2:end)]);
  ...
  for ii = 1:length(tmp),
    if (tmp(ii).isdir && ~strcmpi(tmp(ii).name,'.') && ~strcmpi(tmp(ii).name,'..') ),
      Dt = [Dt; IDArdir([prepath tmp(ii).name filesep wildpath postpath])];
    end;
  end;
  D = [D; Dt];
  ...
  for ii = 1:length(tmp),
    if (tmp(ii).isdir && ~strcmpi(tmp(ii).name,'.') && ~strcmpi(tmp(ii).name,'..') ),
      D = [D; IDArdir([prepath tmp(ii).name postpath])];
    end;
  end;
end;
